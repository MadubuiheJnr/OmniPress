...
export function bootstrapContainer() {
    ...
    const articleCategoryService = new ArticleCategoryService(articleCategoryRepository);
    // 3. Use Cases
    ...
    const createArticleUseCase = new CreateArticleUseCase(articleService, articleCategoryService, userService);
    // 4. Controller
    const authController = new AuthController(loginUseCase, registerUseCase, authService, refreshTokenUseCase);
    const uploadController = new UploadController(uploadService);
    const articleController = new ArticleController(createArticleUseCase);
    const articleCategoryController = new ArticleCategoryController(articleCategoryService);
    // 5. Middleware
    ...
    const articleCategoryRouter = CreateArticleCategoryRouter(articleCategoryController, authMiddleware);
    // 7. Subscribers
