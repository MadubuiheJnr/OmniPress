...
export const ARTICLE_UTILS = {
    slugify_title: (title, existingSlug) => {
        let baseSlug = title
            .toLocaleLowerCase()
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
        ...
        if (attempt >= 5)
            throw new InternalServerError("Failed to generate unique slug", "Please try again");
        return fullSlug;
    },
    generate_html: (doc) => {
        return generateHTML(doc, [StarterKit]);
    },
    calc_read_time: (json, wpm = 230) => {
        let textContent = "";
...
//# sourceMappingURL=article.utils.js.map