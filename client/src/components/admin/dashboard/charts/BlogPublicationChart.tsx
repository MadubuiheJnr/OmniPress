...
const BlogPublicationChart = () => {
  ...
  const publishedCount = blogsData.filter((data) => data.isPublished).length;
  const unpublishedCount = blogsData.filter((data) => !data.isPublished).length;
    ...
    <div className="w-full  bg-white p-3 rounded-b-xl drop-shadow-sm transition-all ease-in-out duration-300">
      <div>
        ...
        <p className="text-sm text-neutral-800">
          Overview of published and pending blogs
        </p>
      </div>
...
export default BlogPublicationChart;
